...
template <class T>
command *CreateShutdownCommand(shutdown_receiver *comms,
...
template <>
command *
CreateShutdownCommand<remote_shutdown_command>(shutdown_receiver *comms,
                                               shutdown_receiver *cli) {
...
public:
  virtual void SetUp() {
    ...
    shutdown_command =
        CreateShutdownCommand<T>(comms_shutdown_mock, cli_shutdown_mock);
  }
...
TYPED_TEST_P(TestShutdownCommands,
             WhenExecutingShutdownCmdIfOKThenShutdownCommsAndCli) {
  EXPECT_CALL(*(this->cli_shutdown_mock), shutdown())
      .Times(1)
      .WillOnce(Return(0));
  EXPECT_CALL(*(this->comms_shutdown_mock), shutdown())
      .Times(1)
      .WillOnce(Return(0));
...
REGISTER_TYPED_TEST_SUITE_P(
    TestShutdownCommands, WhenExecutingShutdownCmdIfOKThenShutdownCommsAndCli);

using ShutdownTestingTypes =
    ::testing::Types<local_shutdown_command, remote_shutdown_command>;
INSTANTIATE_TYPED_TEST_SUITE_P(TestCommands, TestShutdownCommands,
