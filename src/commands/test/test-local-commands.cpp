...
template <>
command *
CreateLocalCommand<local_shutdown_command>(local_command_receiver *cmd_rcv) {
  return new local_shutdown_command(cmd_rcv, nullptr, nullptr);
}

template <>
command *
CreateLocalCommand<local_fast_command>(local_command_receiver *cmd_rcv) {
  return new local_fast_command(cmd_rcv);
}

template <>
command *
CreateLocalCommand<local_slow_command>(local_command_receiver *cmd_rcv) {
  return new local_slow_command(cmd_rcv);
}

template <class T>
void CreateLocalCmdRcvExpectation(LocalCommandReceiverMock *cmd_rcv);

template <>
void CreateLocalCmdRcvExpectation<local_shutdown_command>(
    LocalCommandReceiverMock *cmd_rcv) {
  EXPECT_CALL(*cmd_rcv, send_shutdown_cmd()).Times(1).WillOnce(Return(0));
}

template <>
void CreateLocalCmdRcvExpectation<local_fast_command>(
    LocalCommandReceiverMock *cmd_rcv) {
  EXPECT_CALL(*cmd_rcv, send_fast_cmd()).Times(1).WillOnce(Return(0));
}

template <>
void CreateLocalCmdRcvExpectation<local_slow_command>(
    LocalCommandReceiverMock *cmd_rcv) {
...
REGISTER_TYPED_TEST_SUITE_P(TestLocalCommands,
                            WhenExecutingLocalCmdIfOKThenSendCommand);

using ShutdownTestingTypes =
    ::testing::Types<local_shutdown_command, local_fast_command,
                     local_slow_command>;
INSTANTIATE_TYPED_TEST_SUITE_P(TestCommands, TestLocalCommands,
                               ShutdownTestingTypes);
