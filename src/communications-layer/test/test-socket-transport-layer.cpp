...
ssize_t write_mock(__attribute__((unused)) int fd, const void *buf,
                   size_t count) {
...
TEST_F(TestSocketTransportLayer, WhenSendingIfOKThenMessageIsSent) {
  ...
  EXPECT_CALL(*comms_layer_mock, send(_, sizeof(expected_message)))
      .With(Args<0, 1>(
          ElementsAreArray(expected_message, sizeof(expected_message))))
      .Times(1)
      .WillOnce(Return(sizeof(expected_message)));
  ASSERT_EQ(sizeof(expected_message),
            layer->send(expected_message, sizeof(expected_message)));
  ASSERT_EQ(expected_socket_fd, write_fake.arg0_val);
  ASSERT_EQ(sizeof(expected_message), write_fake.arg2_val);
  ASSERT_THAT(expected_message,
              ElementsAreArray(message_buffer, sizeof(expected_message)));
}
...
TEST_F(TestSocketTransportLayer,
       WhenReceveingIfOKThenMessageIsPassedToNextHandler) {
  ASSERT_EQ(0, layer->connect_socket(SOCKET_NAME));
  EXPECT_CALL(*comms_layer_mock, recv(_, sizeof(message_buffer)))
      .Times(1)
      .WillOnce(Return(sizeof(expected_message)));
  ASSERT_EQ(sizeof(expected_message),
            layer->recv(message_buffer, sizeof(message_buffer)));
  ASSERT_THAT(expected_message,
              ElementsAreArray(message_buffer, sizeof(expected_message)));
...
TEST_F(TestSocketTransportLayer, WhenListeningIfStopListenThenShouldStop) {
  ...
  std::thread t1(&socket_transport_layer::listen_connections, layer,
                 SOCKET_NAME, comms_layer_observer_mock);
