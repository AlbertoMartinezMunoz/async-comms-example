...
TEST_F(TestSocketComms, WhenReceivingIfOKThenReadAndReturnReadBytes)
{
    int expected_socket_id = 5;
    char buffer[32];
    ASSERT_EQ(sizeof(expected_read_buf), layer->receive(expected_socket_id, buffer, sizeof(buffer)));
...
TEST_F(TestSocketComms, WhenReceivingIfReadErrorThenReturnError)
{
    ...
    ASSERT_EQ(-1, layer->receive(expected_socket_id, buffer, sizeof(buffer)));
}
