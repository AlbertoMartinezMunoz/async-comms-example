...
public:
  TestCommandManagerParams(
      std::string name,
      CommandManagerWrapper::SendSimpleCommandSignature send_cmd_func,
      const char *expected_cmd, size_t expected_cmd_size,
      ssize_t send_expected_result, const char *expected_response,
      size_t expected_response_size, ssize_t recv_expected_result,
      int expected_result)
      : name(name), send_cmd_func(send_cmd_func), expected_cmd(expected_cmd),
        expected_cmd_size(expected_cmd_size),
        expected_response(expected_response),
        expected_response_size(expected_response_size),
        expected_result(expected_result),
        send_expected_result(send_expected_result),
...
public:
  TestCommandManagerFastCmdParams(std::string name,
                                  ssize_t send_expected_result,
                                  const char *expected_response,
                                  size_t expected_response_size,
                                  ssize_t recv_expected_result,
                                  int expected_result)
      : TestCommandManagerParams(
            name, &command_manager::send_fast_cmd, command_manager::fast_cmd,
            sizeof(command_manager::fast_cmd), send_expected_result,
            expected_response, expected_response_size, recv_expected_result,
            expected_result) {}
};

class TestCommandManagerShutdownCmdParams : public TestCommandManagerParams {
public:
  TestCommandManagerShutdownCmdParams(std::string name,
                                      ssize_t send_expected_result,
                                      const char *expected_response,
                                      size_t expected_response_size,
                                      ssize_t recv_expected_result,
                                      int expected_result)
      : TestCommandManagerParams(name, &command_manager::send_shutdown_cmd,
                                 command_manager::shutdown_cmd,
                                 sizeof(command_manager::shutdown_cmd),
                                 send_expected_result, expected_response,
                                 expected_response_size, recv_expected_result,
                                 expected_result) {}
};

class TestCommandManager
    : public testing::TestWithParam<TestCommandManagerParams> {
public:
  virtual void SetUp() {
    communications_layer_mock = new CommunicationsLayerMock();
    socket_mock = new SocketTransportLayerMock();
    command_mng =
        new CommandManagerWrapper(communications_layer_mock, socket_mock);
...
TEST_P(TestCommandManager, TestNextSendHandlerMessage) {
  ...
  EXPECT_CALL(*communications_layer_mock, send(StrEq(GetParam().expected_cmd),
                                               GetParam().expected_cmd_size))
      .Times(1)
      .WillOnce(Return(GetParam().send_expected_result));

  if (GetParam().send_expected_result ==
      (ssize_t)(GetParam().expected_cmd_size)) {
    EXPECT_CALL(*communications_layer_mock, recv(_, _))
        .Times(1)
        .WillOnce(
            DoAll(SetArrayArgument<0>(GetParam().expected_response,
                                      GetParam().expected_response +
                                          GetParam().expected_response_size),
                  Return(GetParam().recv_expected_result)));
  }

  CommandManagerWrapper::SendSimpleCommandSignature send_cmd_func =
      GetParam().send_cmd_func;
...
INSTANTIATE_TEST_SUITE_P(
    , TestCommandManager,
    ::testing::Values(
        TestCommandManagerFastCmdParams{
            "WhenSendingFastCommandIfAckThenSendAndReturnAck",
            sizeof(command_manager::fast_cmd), command_manager::ack,
            sizeof(command_manager::ack), sizeof(command_manager::ack), 0},
        TestCommandManagerFastCmdParams{
            "WhenSendingFastCommandIfNackThenSendAndReturnNack",
            sizeof(command_manager::fast_cmd), command_manager::nack,
            sizeof(command_manager::nack), sizeof(command_manager::nack), 1},
        TestCommandManagerFastCmdParams{
            "WhenSendingFastCommandIfResponseErrorThenReturnError",
            sizeof(command_manager::fast_cmd), "BAD", sizeof("BAD"),
            sizeof("BAD"), -1},
        TestCommandManagerFastCmdParams{
            "WhenSendingFastCommandIfSendInternalErrorThenReturnError", -3,
            nullptr, 0, 0, -1},
        TestCommandManagerFastCmdParams{
            "WhenSendingFastCommandIfRecvInternalErrorThenReturnError",
            sizeof(command_manager::fast_cmd), nullptr, 0, -3, -3},
        TestCommandManagerShutdownCmdParams{
            "WhenSendingShutdownCommandIfAckThenSendAndReturnAck",
            sizeof(command_manager::shutdown_cmd), command_manager::ack,
            sizeof(command_manager::ack), sizeof(command_manager::ack), 0},
        TestCommandManagerShutdownCmdParams{
            "WhenSendingShutdownCommandIfNackThenSendAndReturnNack",
            sizeof(command_manager::shutdown_cmd), command_manager::nack,
            sizeof(command_manager::nack), sizeof(command_manager::nack), 1},
        TestCommandManagerShutdownCmdParams{
            "WhenSendingShutdownCommandIfResponseErrorThenReturnError",
            sizeof(command_manager::shutdown_cmd), "BAD", sizeof("BAD"),
            sizeof("BAD"), -1},
        TestCommandManagerShutdownCmdParams{
            "WhenSendingShutdownCommandIfSendInternalErrorThenReturnError", -3,
            nullptr, 0, 0, -1},
        TestCommandManagerShutdownCmdParams{
            "WhenSendingShutdownCommandIfRecvInternalErrorThenReturnError",
...
TEST_F(TestCommandManager, WhenSendingSlowCommandIfAckThenSendAndReturnAck) {
  EXPECT_CALL(
      *communications_layer_mock,
      send(StrEq(command_manager::slow_cmd), sizeof(command_manager::slow_cmd)))
      .Times(1)
      .WillOnce(Return(sizeof(command_manager::slow_cmd)));
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(DoAll(SetArrayArgument<0>(command_manager::ack,
                                          command_manager::ack +
                                              sizeof(command_manager::ack)),
                      Return(sizeof(command_manager::ack))));

  ASSERT_EQ(0, command_mng->send_slow_cmd(slow_response_buffer,
                                          sizeof(slow_response_buffer)));
}
...
TEST_F(TestCommandManager, WhenSendingSlowCommandIfNackThenSendAndReturnNack) {
  ...
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(DoAll(SetArrayArgument<0>(command_manager::nack,
                                          command_manager::nack +
                                              sizeof(command_manager::nack)),
                      Return(sizeof(command_manager::nack))));

  ASSERT_EQ(1, command_mng->send_slow_cmd(slow_response_buffer,
                                          sizeof(slow_response_buffer)));
}

TEST_F(TestCommandManager,
       WhenSendingSlowCommandIfSendInternalErrorThenReturnError) {
...
TEST_F(TestCommandManager,
       WhenSendingSlowCommandIfRecvInternalErrorThenReturnError) {
  int expected_error = -3;
  EXPECT_CALL(
      *communications_layer_mock,
      send(StrEq(command_manager::slow_cmd), sizeof(command_manager::slow_cmd)))
  ...
  ASSERT_EQ(expected_error,
            command_mng->send_slow_cmd(slow_response_buffer,
                                       sizeof(slow_response_buffer)));
}

TEST_F(TestCommandManager,
       WhenSendingSlowCommandIfResponseErrorThenReturnError) {
  ...
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(DoAll(SetArrayArgument<0>(bad_response,
                                          bad_response + sizeof(bad_response)),
