...
TEST_F(TestCommandManager,
       WhenReceivingFastCommandIfAckThenResponseAndReturnAck) {
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(
          DoAll(SetArrayArgument<0>(command_manager::fast_cmd,
                                    command_manager::fast_cmd +
                                        sizeof(command_manager::fast_cmd)),
                Return(sizeof(command_manager::fast_cmd))));
  EXPECT_CALL(*fast_cmd_observer_mock, process_command())
      .Times(1)
      .WillOnce(Return(0));
  EXPECT_CALL(*communications_layer_mock,
              send(StrEq(command_manager::ack), sizeof(command_manager::ack)))
      .Times(1)
...
TEST_F(TestCommandManager,
       WhenReceivingSlowCommandIfAckThenResponseAndReturnAck) {
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(
          DoAll(SetArrayArgument<0>(command_manager::slow_cmd,
                                    command_manager::slow_cmd +
                                        sizeof(command_manager::slow_cmd)),
                Return(sizeof(command_manager::slow_cmd))));
  EXPECT_CALL(*slow_cmd_observer_mock, process_command())
      .Times(1)
      .WillOnce(Return(0));
  ;
  EXPECT_CALL(*communications_layer_mock,
...
TEST_F(TestCommandManager,
       WhenReceivingFastCommandIfNAckThenResponseAndReturnNAck) {
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(
          DoAll(SetArrayArgument<0>(command_manager::fast_cmd,
  ...
  EXPECT_CALL(*fast_cmd_observer_mock, process_command())
      ...
      .WillOnce(Return(1));
  EXPECT_CALL(*communications_layer_mock,
              send(StrEq(command_manager::nack), sizeof(command_manager::nack)))
      .Times(1)
...
TEST_F(TestCommandManager,
       WhenReceivingSlowCommandIfNAckThenResponseAndReturnNAck) {
  EXPECT_CALL(*communications_layer_mock, recv(_, _))
      .Times(1)
      .WillOnce(
          DoAll(SetArrayArgument<0>(command_manager::slow_cmd,
