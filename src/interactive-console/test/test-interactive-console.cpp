...
int pselect_mock(__attribute__((unused)) int nfds,
                 __attribute__((unused)) fd_set *readfds,
                 __attribute__((unused)) fd_set *writefds,
                 __attribute__((unused)) fd_set *exceptfds,
...
public:
  ...
  typedef void (InteractiveConsoleWrapper::*SetCommand)(
      interactive_console_command *);
};

class TestInteractiveConsoleParams {
public:
  TestInteractiveConsoleParams(
      std::string name, const char *console_cmd,
      InteractiveConsoleWrapper::SetCommand set_command_func)
      : name(name), console_cmd(console_cmd),
        set_command_func(set_command_func) {}
...
class TestInteractiveConsole
    : public testing::TestWithParam<TestInteractiveConsoleParams> {
public:
  virtual void SetUp() {
    RESET_FAKE(readline);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        TestInteractiveConsoleParams{
            "WhenReceivesShutdownCommandIfOKThenExecutesTheShutdownCommand",
            "D", &interactive_console::set_shutdown_command},
        TestInteractiveConsoleParams{
            "WhenReceivesFastCommandIfOKThenExecutesTheFastCommand", "F",
            &interactive_console::set_fast_command},
        TestInteractiveConsoleParams{
            "WhenReceivesSlowCommandIfOKThenExecutesTheSlowCommand", "S",
            &interactive_console::set_slow_command}),
    [](const testing::TestParamInfo<TestInteractiveConsoleParams> &info) {
      return info.param.name;
    });

TEST_F(TestInteractiveConsole,
       WhenListeningIfStopThenStopListeningAndShutdown) {
  std::thread t1(&interactive_console::listen, console,
                 std::ref(observer_mock));
