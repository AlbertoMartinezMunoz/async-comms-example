...
class FastCmdObseverMock : public remote_command_observer
{
public:
    MOCK_METHOD(int, process_command, ());
};

class SlowCmdObseverMock : public remote_command_observer
{
public:
    MOCK_METHOD(int, process_command, ());
};
...
public:
    virtual void SetUp()
    {
        ...
        command_mng = new remote_command_manager(communications_layer_mock);
...
protected:
    remote_command_manager *command_mng;
...
TEST_F(TestCommandManager, WhenReceivingFastCommandIfAckThenResponseAndReturnAck)
{
    EXPECT_CALL(*communications_layer_mock, recv(_, _))
        .Times(1)
        .WillOnce(DoAll(SetArrayArgument<0>(remote_command_manager::fast_cmd, remote_command_manager::fast_cmd + sizeof(remote_command_manager::fast_cmd)), Return(sizeof(remote_command_manager::fast_cmd))));
    EXPECT_CALL(*fast_cmd_observer_mock, process_command()).Times(1).WillOnce(Return(0));
    EXPECT_CALL(*communications_layer_mock, send(StrEq(remote_command_manager::ack), sizeof(remote_command_manager::ack)))
        .Times(1)
        .WillOnce(Return(sizeof(remote_command_manager::ack)));
...
TEST_F(TestCommandManager, WhenReceivingSlowCommandIfAckThenResponseAndReturnAck)
{
    EXPECT_CALL(*communications_layer_mock, recv(_, _))
        .Times(1)
        .WillOnce(DoAll(SetArrayArgument<0>(remote_command_manager::slow_cmd, remote_command_manager::slow_cmd + sizeof(remote_command_manager::slow_cmd)), Return(sizeof(remote_command_manager::slow_cmd))));
...
TEST_F(TestCommandManager, WhenReceivingFastCommandIfNAckThenResponseAndReturnNAck)
{
    ...
    EXPECT_CALL(*communications_layer_mock, send(StrEq(remote_command_manager::nack), sizeof(remote_command_manager::nack)))
        .Times(1)
        .WillOnce(Return(sizeof(remote_command_manager::nack)));
    ASSERT_EQ(0, command_mng->incoming_message());
}
